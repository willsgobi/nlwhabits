...
export function Habit() {
    ...
    async function fetchHabits() {
        try {
            ...
            console.log(e)
            Alert.alert("Ops", "Não foi possível obter os hábitos deste dia.")
        } finally {
            setLoading(false)
        }
    }
                ...
                })}>
                    {
                        dayInfo?.possibleHabits ?
                            dayInfo?.possibleHabits.map(habit => {
                                return <CheckBox onPress={() => handleToggleHabit(habit.id)} disabled={isDateInPast} key={habit.id} title={habit.title} checked={completedHabits.includes(habit.id)} />
                            }
                            ) :
                            <HabitEmpty />
                    }
